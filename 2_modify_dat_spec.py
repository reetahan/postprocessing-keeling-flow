...
for scenario_num in range(total_scenario):
    ...
    ss_option = True
    dust_option = True
    util.modify_aero_emit_dist(directory, matrix, ss_option, dust_option)
    
    DMS_option = True
    util.modify_gas_emit(directory, matrix, DMS_option)
    
    util.modify_temp(directory, matrix, ss_option)
